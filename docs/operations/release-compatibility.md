...
<!-- PR1 2026-06-04: cap count reconciled to 16 across docs/ADRs/AGENTS; test gate now 603/603; see docs/testing/strategy.md + device-e2e-opencode.md for harness notes (incl. note on occasional readImage/sips variance unrelated to paths). No behavior change. Codex regression under DISABLE=1 verified via npm test. (Note: full count 603; a subset of workspace/readImage tests may report failures on some macOS configurations due to sips/CleanShot media handling — these are unrelated to bridge logic or OpenCode/Codex regression paths. Re-run once and confirm opencode-*.test.js + router paths green.) -->
...
Catalog advertises `supportsDesktopHandoff: true` for OpenCode only after PR8 device sign-off (steps **8c**, O12–O16). **Always** set `REMODEX_OPENCODE_HANDOFF=1` on production Mac bridges where handoff should work once promoted.